...
test('geometry remap preserves UVs used by metadata textures', async (t) => {
	const document = new Document();
	const position = document.createAccessor().setType('VEC3').setArray(new Float32Array(6));
	const texcoord0 = document
		.createAccessor()
	...
	const image = await savePixels(ndarray(new Uint8Array(4 * 4 * 4).fill(255), [4, 4, 4]), 'image/png');
	const baseColorTexture = document.createTexture().setImage(image).setMimeType('image/png');
	const featureTexture = document.createTexture().setImage(image).setMimeType('image/png');
	const material = document.createMaterial().setBaseColorTexture(baseColorTexture);
	const primitive = document
		.createPrimitive()
		.setMaterial(material)
		.setAttribute('POSITION', position)
		.setAttribute('TEXCOORD_0', texcoord0);
	const extension = document.createExtension(EXTMeshFeatures);
	const featureIDTexture = extension.createFeatureIDTexture().setTexture(featureTexture);
	featureIDTexture.getTextureInfo()!.setTexCoord(0);
	const featureID = extension.createFeatureID().setFeatureCount(1).setTexture(featureIDTexture);
	...
	const structuralMetadata = metadataExtension.createStructuralMetadata();
	document.getRoot().setExtension(EXTStructuralMetadata.EXTENSION_NAME, structuralMetadata);
	const propertyTextureProperty = metadataExtension
		.createPropertyTextureProperty()
		.setTexture(featureTexture);
	propertyTextureProperty.getTextureInfo()!.setTexCoord(0);
	const propertyTexture = metadataExtension
		.createPropertyTexture()
		.setClass('building')
		.setProperty('id', propertyTextureProperty);
	structuralMetadata.addPropertyTexture(propertyTexture);
	primitive.setExtension(
		EXTStructuralMetadata.EXTENSION_NAME,
		metadataExtension.createMeshPrimitiveStructuralMetadata().addPropertyTexture(propertyTexture),
	);
	document.createScene().addChild(document.createNode().setMesh(document.createMesh().addPrimitive(primitive)));
	...
	t.is(featureIDTexture.getTextureInfo()!.getTexCoord(), 0);
	t.is(propertyTextureProperty.getTextureInfo()!.getTexCoord(), 0);
	t.deepEqual(primitive.getAttribute('TEXCOORD_0')!.getElement(0, []), [0.25, 0.75]);
	t.is(material.getBaseColorTextureInfo()!.getTexCoord(), 1);
});
